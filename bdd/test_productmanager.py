...
class ProductManager:
    ...
    def get_all_products_from_db(self):
        ...
        for product in Product.select().order_by(fn.Rand()).limit(10): # get randomly 10 products from the db
    ...
    def get_products_from_category(self, category):
        ...
        for product in Product.select().join(Category)\
            ...
            .order_by(fn.Rand()).limit(10):
                ...
                print(product.product_name, "--> le nutriscore est : ", product.nutrition_grade_fr.upper())  

                #ça renvoie un résultat en STRING type(product.product_name) >>> <class' str'>
    ...
    def get_healthy_products_from_category(self, category):
        ...
        for product in Product.select().join(Category) \
        .where(Category.category_name == self.query)\
        .order_by(Product.nutrition_grade_fr).limit(10):
...
productmanager.get_unhealthy_products("Pizza")
productmanager.get_healthy_products("Pizza")
